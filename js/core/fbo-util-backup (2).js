      ...
      gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, width, height, 0, gl.RGBA, gl.FLOAT, null);
        // for(var i = 0; i < height; ++i)
         //   for(var j = 0; j < width; ++j)
          //      noiseTex[j + i*width] = [1.0, 1.0, 1.0];
        ...
        if (multipleTargets) {
          ...
          gl.framebufferTexture2D( gl.FRAMEBUFFER, drawbuffers[3], gl.TEXTURE_2D, textures[3], 0 );
            gl.framebufferTexture2D( gl.FRAMEBUFFER, noiseTex, gl.TEXTURE_2D, textures[5], 0 );
          ...
          if (FBOstatus !== gl.FRAMEBUFFER_COMPLETE) {
            ...
            gl.bindFramebuffer(gl.FRAMEBUFFER, null);
            // Set up GBuffer Noise
            fbo[FBO_GBUFFER_NOISE] = gl.createFramebuffer();
            gl.bindFramebuffer(gl.FRAMEBUFFER, fbo[FBO_GBUFFER_NOISE]);
            gl.framebufferTexture2D(gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0, gl.TEXTURE_2D, textures[5], 0);

            FBOstatus = gl.checkFramebufferStatus(gl.FRAMEBUFFER);
            if (FBOstatus !== gl.FRAMEBUFFER_COMPLETE) {
                console.log("GBuffer Noise FBO incomplete! Init failed!");
                return false;
            }
        }
