...
var main = function (canvasId, messageId) {
    ...
    stats_ms.domElement.style.position = 'relative';
    stats_ms.domElement.style.top = '49px';
...
var render = function () {
    ...
    if (!isDiagnostic) { //key 0 to run the pipeline
        renderShade();
        renderPost();
    } else {
        renderDiagnostic(); //key 1~8 to diagnos
    }
...
var renderDiagnostic = function () {
    ...
    // Bind necessary textures
    gl.activeTexture(gl.TEXTURE0);  //position, key 1
    gl.bindTexture(gl.TEXTURE_2D, fbo.texture(0));
    gl.uniform1i(diagProg.uPosSamplerLoc, 0);

    gl.activeTexture(gl.TEXTURE1);  //normal, key 2
    gl.bindTexture(gl.TEXTURE_2D, fbo.texture(1));
    gl.uniform1i(diagProg.uNormalSamplerLoc, 1);

    gl.activeTexture(gl.TEXTURE2);  //color, key 3
    gl.bindTexture(gl.TEXTURE_2D, fbo.texture(2));
    gl.uniform1i(diagProg.uColorSamplerLoc, 2);

    gl.activeTexture(gl.TEXTURE3);  //depth, key 4
    gl.bindTexture(gl.TEXTURE_2D, fbo.depthTexture());
...
var initShaders = function () {
    if (fbo.isMultipleTargets()) {
        ...
        passProg.addCallback(function () {
            ...
            passProg.u_timeLocation = gl.getUniformLocation(passProg.ref(), "u_time");
