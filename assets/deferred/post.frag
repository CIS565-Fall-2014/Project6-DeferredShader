...
/*
	...
	*/
	...
	for(int i=0;i<SAMPLEKERNEL_SIZE;i++){
		...
		float sdp=texture2D(u_depthTex,v_texcoord+vec2(sample.x,sample.y)*u_radius).r;
		sdp=linearizeDepth(sdp,u_zNear,u_zFar);
		
		float sz=ori.z-(sample*u_radius).z/2.0;
		
		float range=abs(ori.z-sdp)<u_radius?1.0:0.0;
		
		if(sdp<=sz) occ+=range;
	}
	occ=1.0-occ/float(SAMPLEKERNEL_SIZE);
