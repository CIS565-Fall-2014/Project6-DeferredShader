...
void main()
{
  ...
  // int lightNum = 1;
   ...
   u_displayType == DISPLAY_BLOOM || 
   u_displayType == DISPLAY_SSAO || 
   u_displayType == DISPLAY_BLUR)
{
//BLINN-PHONG
for(int l = 0; l < 2; l++)
	{
	 ...
	 // vec3 lightDir = myNormalize( lightpos - myPos );
	  float dist = length(lightDir) * length(lightDir);
		...
		colorSum += (lights[l].emit * lights[l].color * (0.7 * diffuseColor + 0.3 * specularColor))/1.0;
