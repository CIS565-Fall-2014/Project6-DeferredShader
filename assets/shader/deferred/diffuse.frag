...
void main()
{
  ...
  vec2 rand;
  rand.x = v_texcoord.x;
  rand.y = 1.0 - (rand.x * rand.x);
  
  vec2 left  = normalize(vec2(-rand.x, rand.y));
  vec2 right = normalize(vec2(rand.x , rand.y));
  vec2 up    = normalize(vec2(rand.y, rand.x));
  vec2 down  = normalize(vec2(rand.y, -rand.x));
  */
